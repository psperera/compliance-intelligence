...
export function AiImpact({ initial, source, auto = true }: { initial: Analysis; source: Source; auto?: boolean }) {
  ...
  const generate = useCallback(async () => {
    setBusy(true);
    try {
      const res = await fetch("/api/impact", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(source) });
  ...
  useEffect(() => { if (auto) generate(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, []);
  ...
  return (
    <div className="aibox">
      ...
      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 8, borderTop: "1px solid #e0d8f5", paddingTop: 8, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        ...
        <button className="btn sm" style={{ marginLeft: "auto" }} disabled={busy} onClick={generate}>{busy ? "Generating…" : "Regenerate with LLM"}</button>
      </div>
    </div>
  );
}
